...
if (isset($_GET['employee_spotlight_optin'])) {
	if (!function_exists('wp_get_current_user')) {
		require_once (ABSPATH . 'wp-includes/pluggable.php');
	}
	if ($_GET['employee_spotlight_optin'] == 1) {
		//opt-in
	...
	wp_redirect(esc_url(remove_query_arg('employee_spotlight_optin')));
	exit;
} else {
...
function employee_spotlight_show_optin() {
	...
	if (!get_option('employee_spotlight_tracking_optin')) {
		...
		$tr_link = implode(' ', array(
			sprintf(__('%sDo not allow%s', 'employee-spotlight') , '<a href="' . admin_url('admin.php?page=employee_spotlight&employee_spotlight_optin=0') . '" class="button-secondary" id="employee-spotlight-do-not-allow-tracking">', '</a>') ,
			sprintf(__('%sAllow%s', 'employee-spotlight') , '<a href="' . admin_url('admin.php?page=employee_spotlight&employee_spotlight_optin=1') . '" class="button-primary" id="employee-spotlight-allow-tracking">', '</a>') ,
		));
		...
		echo '<li class="emd-permission"><i class="dashicons dashicons-email-alt"></i><div><span>' . __('Newsletter', 'employee-spotlight') . '</span><p>' . __('Updates, announcements, marketing, no spam', 'employee-spotlight') . ', <a href="https://emdplugins.com/subscription-preferences/" target="_blank">unsubscribe anytime</a></p></div></li>';
		echo '</ul></div></div>';
	} else {
...
function employee_spotlight_show_rateme_action() {
	...
	if (5 === $min_trigger) {
		...
		$response['redirect'] = "https://emdplugins.com/plugin_tag/empslight-com";
		$min_trigger = - 1;
	}
...
function employee_spotlight_show_rateme_notice() {
	...
	if ($min_count > 5 || ($min_trigger == 5 && $installed_date <= $today)) {
		$message_start = '<div class="emd-show-rateme update-nag success">
                        <span class=""><b>Employee Spotlight</b></span>
                        <div>';
		if ($min_count > 5) {
			$message_start.= sprintf(__("Hi, I noticed you just crossed the %d %s milestone on Employee Spotlight - that's awesome!", "employee-spotlight") , $min_trigger, $label);
		...
		$message_level1 = __('Could you please do me a <span style="color:red" class="dashicons dashicons-heart"></span> BIG favor <span style="color:red" class="dashicons dashicons-heart"></span> and give it a 5-star rating on WordPress? Just to help us spread the word and boost our motivation.', "employee-spotlight");
		...
		$message_end1 = '<li><a data-rate-action="do-rate" data-plugin="employee_spotlight" href="#">' . __('Ok, you deserve it', 'employee-spotlight') . '</a>
       </li>
        <li><a data-rate-action="done-rating" data-plugin="employee_spotlight" href="#">' . __('I already did', 'employee-spotlight') . '</a></li>
...
function employee_spotlight_plugin_row_meta($input, $file) {
	...
	$links = array(
		...
		'<a href="https://emdplugins.com/plugin_tag/empslight-com">' . __('Pro Version', 'employee-spotlight') . '</a>'
	);
...
function employee_spotlight_deactivation_feedback_box() {
	...
	foreach ($reasons as $key => $reason) {
		...
		switch ($key) {
			...
			case 9:
				$feedback_vars['reasons'].= 'data-input-type="textarea"';
				$feedback_vars['reasons'].= 'data-input-placeholder="' . __('We are sorry to hear that. Please share your previous version number before update, new updated version number and what happened', 'employee-spotlight') . '"';
			break;
			case 5:
				$feedback_vars['reasons'].= 'data-input-type="textarea"';
				$feedback_vars['reasons'].= 'data-input-placeholder="' . __('We are sorry to hear that. Please share what happened', 'employee-spotlight') . '"';
			break;
			case 22:
				$feedback_vars['reasons'].= 'data-input-type="textarea"';
