...
if ( !class_exists( 'PluginUpdateChecker_3_0', false ) ):
	...
	class PluginUpdateChecker_3_0 {
		...
		public function __construct( $metadataUrl, $pluginFile, $slug = '', $checkPeriod = 12, $optionName = '',
			...
			$this->debugMode			 = false; //(bool) (constant( 'WP_DEBUG' ));
		...
		public function requestInfo( $queryArgs = array() ) {
			...
			if ( !is_wp_error( $status ) ) {
				...
				. $status->get_error_message(), E_USER_WARNING
				);
			}
		...
		private function validateApiResponse( $result ) {
			...
			if ( $result[ 'response' ][ 'code' ] !== 200 ) {
				return new WP_Error(
				'puc_unexpected_response_code', 'HTTP response code is ' . $result[ 'response' ][ 'code' ] . ' (expected: 200)'
				);
			}
		...
		public function getInstalledVersion() {
			...
			if ( isset( $pluginHeader[ 'Version' ] ) ) {
				...
				$this->triggerError(
				sprintf(
				"Can't to read the Version header for '%s'. The filename is incorrect or is not a plugin.", $this->pluginFile
				), E_USER_WARNING
				);
				return null;
			}
		...
		protected function getPluginHeader() {
			if ( !is_file( $this->pluginAbsolutePath ) ) {
				...
				$this->triggerError(
				sprintf(
				"Can't to read the plugin header for '%s'. The file does not exist.", $this->pluginFile
				), E_USER_WARNING
				);
				return array();
			}
		...
		public function fixDirectoryName( $source, $remoteSource, $upgrader ) {
			...
			if ( $source !== $correctedSource ) {
				...
				if ( $this->isBadDirectoryStructure( $remoteSource ) ) {
					return new WP_Error(
					'puc-incorrect-directory-structure', sprintf(
					'The directory structure of the update is incorrect. All plugin files should be inside ' .
					'a directory named <span class="code">%s</span>, not at the root of the ZIP file.', htmlentities( $this->slug )
					)
					);
				}
				...
				'Renaming %s to %s&#8230;', '<span class="code">' . basename( $source ) . '</span>', '<span class="code">' . $pluginDirectoryName . '</span>'
				) );
				...
				if ( $wp_filesystem->move( $source, $correctedSource, true ) ) {
					...
					return new WP_Error(
					'puc-rename-failed', 'Unable to rename the update to match the existing plugin directory.'
					);
				}
			}
		...
		public function addCheckForUpdatesLink( $pluginMeta, $pluginFile ) {
			$isRelevant = ($pluginFile == $this->pluginFile) || (!empty( $this->muPluginFile ) && $pluginFile == $this->muPluginFile);
		...
		public function displayManualCheckResult() {
			if ( isset( $_GET[ 'puc_update_check_result' ], $_GET[ 'puc_slug' ] ) && ($_GET[ 'puc_slug' ] == $this->slug) ) {
				...
				printf(
				'<div class="updated notice is-dismissible"><p>%s</p></div>', apply_filters( 'puc_manual_check_message-' . $this->slug, $message, $status )
				);
			}
		}
...
if ( !class_exists( 'PluginInfo_3_0', false ) ):
	...
	class PluginInfo_3_0 {
		...
		protected static function validateMetadata( $apiResponse ) {
			if (
			!isset( $apiResponse->name, $apiResponse->version ) || empty( $apiResponse->name ) || empty( $apiResponse->version )
			) {
				return new WP_Error(
				'puc-invalid-metadata', "The plugin metadata file does not contain the required 'name' and/or 'version' keys."
				);
			}
			return true;
		}
...
if ( !class_exists( 'PluginUpdate_3_0', false ) ):
	...
	class PluginUpdate_3_0 {
		...
		public function toWpFormat() {
			...
			$update->package	 = $update->package	 = str_replace( "//?", "/?", $this->download_url );
...
if ( !class_exists( 'PucScheduler_3_0', false ) ):
	...
	class PucScheduler_3_0 {
		...
		public function maybeCheckForUpdates() {
			...
			'puc_check_now-' . $this->updateChecker->slug, $shouldCheck, (!empty( $state ) && isset( $state->lastCheck )) ? $state->lastCheck : 0, $this->checkPeriod
			);
