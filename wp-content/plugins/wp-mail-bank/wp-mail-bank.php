...
if (!defined("mail_bank_version_number")) {
   define("mail_bank_version_number", "3.0.28");
}
...
if (!function_exists("mail_bank_email_logs")) {
...
if ($version >= "3.0.3") {
   ...
   /*
     plugins_loaded for get_users_capabilities_mail_bank
...
function mail_bank_admin_notice_class() {
   ...
   class mail_bank_admin_notices {
      ...
      /**
        ...
        */
       public function get_install_url_mail_bank($plugin,$filename) {
      ...
      public function change_admin_notice_mail_bank($admin_notices) {
         ...
         foreach ($admin_notices as $slug => $admin_notice) {
            ...
            if (!$this->mb_required_fields($admin_notices[$slug])) {
               ...
               $installing_mail_bank = "Mail Bank recommends installing ";
               $install_now_link = " Install Now! ";
               $gallery_portfolios = ") for Galleries / Portfolios";
               $contact_bank_form_builder = ") for Form Builder & Contact Forms";
               $gallery_bank_install_url = $this->get_install_url_mail_bank("gallery-bank","gallery-bank.php");              
               $contact_bank_install_url = $this->get_install_url_mail_bank("contact-bank","contact-bank.php");
               $mail_bank_recommends_plugins = "";
               if(class_exists("gallery_bank_admin_notices") && class_exists("contact_bank_admin_notices"))
               {
                  $mail_bank_recommends_plugins = "";
               }
               else if(!class_exists("gallery_bank_admin_notices") && class_exists("contact_bank_admin_notices"))
               {
                   $mail_bank_recommends_plugins = "<strong> $installing_mail_bank Gallery Bank (</strong><a style=text-decoration:none; href=$gallery_bank_install_url>$install_now_link</a><strong>$gallery_portfolios</strong>.";
               }
               else if(!class_exists("contact_bank_admin_notices") && class_exists("gallery_bank_admin_notices"))
               {
                   $mail_bank_recommends_plugins = "<strong>$installing_mail_bank Contact Bank (</strong><a style=text-decoration:none; href=$contact_bank_install_url >$install_now_link</a><strong>$contact_bank_form_builder</strong>.";
               }
               else if(!class_exists("contact_bank_admin_notices") && !class_exists("gallery_bank_admin_notices"))
               {
                   $mail_bank_recommends_plugins = "<strong>$installing_mail_bank Gallery Bank (</strong><a style=text-decoration:none; href=$gallery_bank_install_url>$install_now_link</a><strong>$gallery_portfolios <br/>$installing_mail_bank Contact Bank (</strong><a style=text-decoration:none; href=$contact_bank_install_url>$install_now_link</a><strong>$contact_bank_form_builder</strong>.";
               }
               $output_css = false;
               ...
               if ($admin_display_check == 0 && strtotime($admin_display_start) <= strtotime($current_date)) {
                  ...
                  if (strpos($slug, 'promo') === FALSE) {
                     ...
                     echo $mail_bank_recommends_plugins;
                     echo '</div>';
                  }
                  $this->notice_spam += 1;
                  $output_css = true;
      ...
      public function mb_display_admin_notices() {
         ...
         $notices['two_week_review'] = array(
             'title' => 'Leave A Mail Bank Review?',
             ...
             'link' => '<span class="dashicons dashicons-external mail-bank-admin-notice"></span><span class="mail-bank-admin-notice"><a href="https://wordpress.org/support/plugin/wp-mail-bank/reviews/?filter=5" target="_blank" class="mail-bank-admin-notice-link">' . 'Sure! I\'d love to!' . '</a></span>
                        <span class="dashicons dashicons-smiley mail-bank-admin-notice"></span><span class="mail-bank-admin-notice"><a href="' . $two_week_review_ignore . '" class="mail-bank-admin-notice-link"> ' . 'I\'ve already left a review' . '</a></span>
                        <span class="dashicons dashicons-calendar-alt mail-bank-admin-notice"></span><span class="mail-bank-admin-notice"><a href="' . $two_week_review_temp . '" class="mail-bank-admin-notice-link">' . 'Maybe Later' . '</a></span>',
