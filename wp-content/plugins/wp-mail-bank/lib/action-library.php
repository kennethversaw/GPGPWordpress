      ...
      if (isset($_REQUEST["param"])) {
         ...
         switch (esc_attr($_REQUEST["param"])) {
               ...
               if (wp_verify_nonce(isset($_REQUEST["_wp_nonce"]) ? $_REQUEST["_wp_nonce"] : "", "mail_bank_test_email_configuration")) {
                  ...
                  if ($mailer_type_mail_bank == "smtp") {
                     ...
                     if ($settings_data_array["debug_mode"] == "enable") {
                        if (get_option("mail_bank_is_mail_sent") != "Sent") {
                           ...
                           if (stripos($mail_bank_mail_status, "socket") !== false) {
                              $debugging_output .= "Your host may have installed a firewall between you and the server. Ask them to open the ports.\nYou may have tried to (incorrectly) use SSL over port 587. Check your encryption and port settings.\n";
                           } else if (stripos($mail_bank_mail_status, "Timed") !== false) {
                              $debugging_output .= " Your host may have poor connectivity to the mail server. Try doubling the Read Timeout.\n Your host may have installed a firewall (DROP packets) between you and the server. Ask them to open the ports.\n Your host may have tried to (incorrectly) use TLS over port 465. Check your encryption and port settings.\n";
                           } else if (stripos($mail_bank_mail_status, "Connection") !== false) {
                              $debugging_output .= "Your host has likely installed a firewall (REJECT packets) between you and the server. Ask them to open the ports.\n";
                           } else {
                     ...
                     if ($monitor_email_logs == "enable") {
                        ...
                        $email_logs_data["email_data"] = serialize($email_logs_data_array);
                        $obj_dbHelper_mail_bank->insertCommand(mail_bank_email_logs(), $email_logs_data);
                     }
                     if ($result != "true" || $result != "1") {
            ...
            case "mail_bank_roles_and_capabilities_module":
               if (wp_verify_nonce(isset($_REQUEST["_wp_nonce"]) ? $_REQUEST["_wp_nonce"] : "", "mail_bank_roles_capabilities")) {
                  parse_str(isset($_REQUEST["data"]) ? base64_decode($_REQUEST["data"]) : "", $roles_array);
                  $update_roles = array();
                  $where = array();
                  ...
                  $update_data["meta_value"] = serialize($unserialized_roles_and_capabilities);
                  $obj_dbHelper_mail_bank->updateCommand(mail_bank_meta(), $update_data, $where);
               }
               break;
         }
         die();
