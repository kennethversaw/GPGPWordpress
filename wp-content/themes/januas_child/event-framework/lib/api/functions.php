...
class EF_Activation_License_Manager {
    ...
    public function load_plugin_self_updater() {
        ...
        //$plugin_name       = strtolower($current_theme->get('Name')); // same as plugin slug. if a theme use a theme name like 'twentyeleven'
        $plugin_name       = strtolower(empty($template) ? str_replace(' ', '-', $current_theme->get('Name')) : $template); // same as plugin slug. if a theme use a theme name like 'twentyeleven'
